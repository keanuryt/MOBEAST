...
generate.long.format.OTUs = function(abund,metadata,taxonomy) {
  ...
  #abund = relative or raw abundance dataframe, with the samples as columns and OTUs as rows. Column names must correspond exactly to sample names in metadata and row names must correspond exactly to OTU names in taxonomy.
  ...
  df1$OTUNumber=rep(rownames(abund),times=ncol(abund)) #add a new column, OTU, which contains the original OTU names ( rownames(abund) ), repeated by the number of columns ( times=ncol(abund) ). Because unlist will list values sequentially BY COLUMN, to create the "OTU" column the original OTU names in the original order are simply repeated X times, where X is the number of columns. This new column will have a length = nrow(abund) x ncol(abund), and now each value in the "abund" column has an associated value in the "OTU" column. 
  
  df1$SampleID=rep(colnames(abund),each=nrow(abund))
  
  df2=merge(df1,taxonomy,by.x="OTUNumber",by.y="OTUNumber") ## change those labels to appropriate colnames
  
  df3=merge(df2,metadata,by.x="SampleID",by.y="id_number")

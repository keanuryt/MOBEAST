...
Data <- pHSlope %>%
  ...
  mutate(DateTime = ymd_hms(paste(Date, Time)), # make a datetime
         ...
         deltaTA = TA - TA_inflow, # calculate the difference between inflow and outflow
...
carbonate<- carbonate %>% select(tank_id, inflow_table, date, time, 
                     ...
                     ta, ta_inflow, delta_ta) %>% 
   mutate(treatment = factor(treatment, # reorder the factor treatments 
                            levels = c("Control","Coral",
                                       "CCA", "Algae"))) %>% 
  mutate(tank_id = paste0("T", tank_id), 
         inflow_table = paste("Table", inflow_table)) %>% 
  rename(tank_number = tank_id )
```

```{r}
inflow_data <- carbonate %>% select(-tank_number, -treatment, -ph,-ph_diff, -ta, -delta_ta) %>% 
  distinct() %>% 
  
  mutate(tank_number = ifelse(inflow_table == "Table 1", "F1", "F2"), 
         treatment = "Inflow", 
         ph = ph_inflow, 
         ph_diff = ph - ph_inflow, 
         ta = ta_inflow,
         delta_ta = ta-ta_inflow)
  
```

```{r}
carbonate <- rbind(carbonate, inflow_data)
```
...
carbonate %>% 
  filter(treatment != "Inflow") %>% 
  drop_na() %>% 
  ggplot(aes(x=date_time, y= delta_ta , color = treatment, group = tank_number))+
  geom_point(size = 2)+
  geom_line(linewidth = 1)+
  #geom_hline(yintercept=0, linetype="dashed", color = "orange", alpha =0.8)+
  #geom_pointrange(aes(ymin= avg-se, ymax= avg+se)) +
...
## Combine with DOC data 

```{r}
carbonate

#write_csv(carbonate, here("data", "carbonate.csv"))
```

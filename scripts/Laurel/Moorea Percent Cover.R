...
coral_cover_br <- read_csv(here("data", "Laurel", "corals.csv"))
...
backreef_mean_cover <- read_csv(here("data","Laurel", "backreef_benthic_comp_mean_perc.csv"))
...
br_benthic_cover_plot <- ggplot(data=backreef_mean_cover, aes(fill=benthic_group, x=year_fact, y=mean_percent_cover)) +
  ...
  labs(x = "Year", y = "Mean Percent Cover", 
       title = "Mo'orea Backreef Benthic Percent Cover", fill = "Benthic Category",
       caption= "Credit: Laurel Diaz, CSUN") +
  theme_bw() +
...
ggsave(plot = br_benthic_cover_plot, filename = here("outputs", "br_benthic_cover_plot.png"), width = 8, height = 10)

##pre cover plot##
backreef_2006 <- read_csv(here("data","Laurel", "backreef_2006_cover.csv"))
...
## COTS cover plot##
backreef_2011 <- read_csv(here("data","Laurel","backreef_2011_cover.csv"))
...
backreef_2019 <- read_csv(here("data","Laurel", "backreef_2019_cover.csv"))
...
backreef_algae <- read_csv(here("data","Laurel", "benthic_algal_cover.csv"))
